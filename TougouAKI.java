...
class TougouAKI {
    ...
    static int turn;
    static int hitEx = 0, hitEy = 0;//命中した敵艦座標、命中した敵艦の移動先(秋山)
    static int cntChase = 0;// 自軍攻撃がヒット(秋山)
    static boolean moveEb = false;//敵が移動したか(秋山)
    static String dir = "";//(秋山)
    static int dist = 0;//宣言する位置の変更(秋山)
        ...
        while (true) {
            ...
            if (turn == 0) {
                ...
                if (action == 0) {
                    ...
                    System.out.print("敵の攻撃位置(y x): ");
                    // int ex = sc.nextInt();
                    // int ey = sc.nextInt();

                    int ey = (int) sc.next().toUpperCase().charAt(0) - 65;
                    int ex = sc.nextInt() - 1;

                    printAttackMessage(ey, ex);

                    enemyAttack(ey, ex);// 0 自軍撃沈、1 命中、2 波高し、3 ハズレ(秋山)
                    if (enemyResult == 1) {
                        hitX = ey;
                        hitY = ex;
                    } // 命中で生き残りの場合、座標記憶(秋山)
    ...
    static void enemyAttack(int x, int y) {
        ...
        if (enemyResult == 1 || enemyResult == 2) { // 波高しのとき相手が攻撃したマスの周囲8マスを+1、そのマスを0
            for (int dx = -1; dx <= 1; dx++) {
                for (int dy = -1; dy <= 1; dy++) {
                    if (dx == 0 && dy == 0){
    ...
    static void playerAttack(int sx, int sy, int tx, int ty) {
        ...
        if(board_enemy[tx][ty] >= 0){
            board_enemy[tx][ty] = 0;//攻撃マスは0にしてしまう(秋山)
        }
        // 敵の応答
        int result = enemyResponse();
        if (result == 0) {
            System.out.println("命中" + "!");
        } else if (result == 1) {
            System.out.println("波高し" + "!");
    ...
    static void playerAttackAuto() { // (濱口)
        ...
        System.out.println("cntChase " + cntChase);
        if(cntChase >= 1){//追撃を追加(秋山)
            board_enemy[hitEx][hitEy] += 1000;
            if(moveEb)moveE_P();
            if(cntChase >= 2) cntChase = -1;
            cntChase += 1;
        }
    ...
    // 敵軍からの攻撃結果を返す(濱口)
    static int enemyResponse() {
        Scanner sc = new Scanner(System.in);
        System.out.print("相手の応答(0:命中 1:波高し 2:撃沈 3:ハズレ):");
