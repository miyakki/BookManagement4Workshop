...
class Undo {
    ...
    // --- Undo用の履歴保存スタック ---
    static Stack<int[][]> historyBoard = new Stack<>();
    static Stack<int[][]> historyEnemy = new Stack<>();
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            // ★ Undoの確認
            System.out.print("今のターンをやり直しますか？ (1:はい / 0:いいえ): ");
            int undoChoice = sc.nextInt();
            if (undoChoice == 1) {
                undo();
                System.out.println(">> ターンを巻き戻しました。");
                // turnはそのまま（同じ陣営のターンからやり直し）
            } else {
                // ターン交代
                turn = (turn == 0) ? 1 : 0;
            }
    ...
    static void saveState() {
        ...
        int[][] currentEnemy = new int[SIZE][SIZE];
        for (int i = 0; i < SIZE; i++) {
            currentBoard[i] = board[i].clone();
            currentEnemy[i] = board_enemy[i].clone();
        }
        historyBoard.push(currentBoard);
        historyEnemy.push(currentEnemy);
    }
